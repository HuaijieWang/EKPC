...
class Adapter(nn.Module):
    def __init__(self, config=None, d_model=None, bottleneck=None, dropout=0.0, init_option="bert",
        ...
        self.bias = nn.Parameter(torch.empty(1, self.n_embd).uniform_(-1, 1))
    ...
    def forward(self, x, add_residual=True, residual=None, weight=None, bias=None, task_id=None, LST=False, idx=0, feature_list=None):
...
class Block(nn.Module):
    ...
    def forward(self, x, weight=None, bias=None, task_id=None,LST=False, idx=None, feature_list=None):
        residual_adapter = x
        x = x + self.drop_path(self.attn(self.norm1(x)))  
        
        if self.config.ffn_adapt and self.config.ffn_option == 'parallel' and self.layer_id <= 12:
            adapt_x, feature_list = self.adaptmlp(x, add_residual=False, weight=weight, bias=bias, task_id=task_id, LST=LST, idx=idx, feature_list = feature_list)  
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, weight=None, bias=None, task_id=None, LST=False):
        B = x.shape[0]  
        ...
        for idx, blk in enumerate(self.blocks): 
            ...
            x, self.feature_list = blk(x, weight=weight, bias=bias, task_id=task_id, LST=False, idx=idx, feature_list=self.feature_list)
            if self.tuning_config.vpt_on:
    ...
    def forward(self, x, weight=None, bias=None, task_id=None, LST=False):
        x = self.forward_features(x, weight=weight, bias=bias, task_id=task_id, LST=LST)  
...
def pretrained_vit_b16_224_dino1k_adapter(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)

    checkpoint_model = timm.create_model("vit_base_patch16_224_dino", pretrained=True, num_classes=0)
    state_dict = checkpoint_model.state_dict()
    for key in list(state_dict.keys()):
        if 'qkv.weight' in key:
            qkv_weight = state_dict.pop(key)
...
def pretrained_vit_b16_224_iBOT1k_adapter(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)

    state_dict = torch.load("/data/whj/SSIAT-main/ibot/checkpoint_teacher.pth")
    for key in list(state_dict.keys()):
        if 'qkv.weight' in key:
            qkv_weight = state_dict.pop(key)
            q_weight = qkv_weight[:768]
