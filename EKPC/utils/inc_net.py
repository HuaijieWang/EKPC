...
def get_convnet(args, pretrained=False):
    name = args["convnet_type"].lower()  # "pretrained_vit_b16_224_in21k_adapter"
    ...
    # DINO-1k
    elif name == "pretrained_vit_b16_224_dino1k" or name == "vit_base_patch16_224_dino":
        ...
        if args["model_name"] == "adapter":
            ...
            elif name == "pretrained_vit_b16_224_dino1k_adapter":
                model = vision_transformer_adapter.pretrained_vit_b16_224_dino1k_adapter(num_classes=0,
                                                                                      global_pool=False,
                                                                                      drop_path_rate=0.0,
                                                                                      tuning_config=tuning_config)
                model.out_dim = 768

            elif name == "pretrained_vit_b16_224_ibot1k_adapter":
                model = vision_transformer_adapter.pretrained_vit_b16_224_iBOT1k_adapter(num_classes=0,
                                                                                      global_pool=False,
                                                                                      drop_path_rate=0.0,
                                                                                      tuning_config=tuning_config)
                model.out_dim = 768
...
class BaseNet(nn.Module):
    ...
    def extract_vector(self, x):
        return self.convnet(x, weight=self.weight,bias=self.bias, task_id=self.task_id, LST=self.LST)["features"]

    def forward(self, x):
        x = self.convnet(x, weight=self.weight,bias=self.bias, task_id=self.task_id, LST=self.LST)
...
class SimpleVitNet(BaseNet):
    ...
    def adapt_feature(self, in_dim=1, out_dim=768, task_id=0):   
    ...
    def extract_vector(self, x):
        return self.convnet(x, weight=self.weight,bias=self.bias, task_id=self.task_id, LST=self.LST)
